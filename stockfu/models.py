...
class SignalScanRun(SQLModel, table=True):
    """一次收盘后策略信号扫描。

    这是可审计的业务批次，不属于可清理的 operator cache。股票池与策略清单按
    当次运行固化，重跑同一交易日会产生新批次，读取端选择最近成功批次。
    """
    __tablename__ = "signal_scan_run"
    id: int | None = Field(default=None, primary_key=True)
    signal_date: date = Field(index=True)
    status: str = Field(default="running", index=True)  # running/success/partial/failed
    index_codes_json: str = "[]"
    universe_json: str = "[]"
    strategy_ids_json: str = "[]"
    universe_size: int = 0
    factor_expected: int = 0
    factor_completed: int = 0
    llm_requested: int = 0
    llm_completed: int = 0
    error: str = ""
...
class FactorSignal(SQLModel, table=True):
    """股票在一次扫描中、某个策略下的完整评分记录。"""
    __tablename__ = "factor_signal"
    id: int | None = Field(default=None, primary_key=True)
    scan_run_id: int = Field(foreign_key="signal_scan_run.id", index=True)
    signal_date: date = Field(index=True)
    asset_code: str = Field(index=True)
    strategy_id: str = Field(index=True)
    strategy_name: str = ""
    strategy_fingerprint: str = ""
    score_full: float = 20.0
    score: float | None = None                    # 面向用户的 0–100 分
    raw_score: float | None = None                # 策略原始聚合分
    confidence: float | None = None
    legacy_signal: str = ""                      # 兼容旧五档标签，不作主要展示
    risk_vetoed: bool = False
    factors_json: str = "{}"
    details_json: str = "[]"                     # 算子置信度/理由等完整审计明细
    error: str = ""
    created_at: datetime = Field(default_factory=_now)
    __table_args__ = (UniqueConstraint(
        "scan_run_id", "asset_code", "strategy_id",
        name="uq_factor_signal_run_code_strategy"),)


class LlmSignalAnalysis(SQLModel, table=True):
    """按需调用 LLM 得到的独立股票分析；不会覆盖因子策略评分。"""
    __tablename__ = "llm_signal_analysis"
    id: int | None = Field(default=None, primary_key=True)
    scan_run_id: int = Field(foreign_key="signal_scan_run.id", index=True)
    signal_date: date = Field(index=True)
    asset_code: str = Field(index=True)
    model: str = ""
    score: float | None = None                    # LLM 独立的 0–100 分
    summary: str = ""
    reasons_json: str = "[]"
    risks_json: str = "[]"
    ...
    created_at: datetime = Field(default_factory=_now)
    __table_args__ = (UniqueConstraint(
        "scan_run_id", "asset_code", name="uq_llm_signal_run_code"),)


class StockSignalSubscription(SQLModel, table=True):
    """逐股推荐订阅；无记录等价于两个开关都关闭。"""
    __tablename__ = "stock_signal_subscription"
    asset_code: str = Field(primary_key=True)
    factor_mail_enabled: bool = Field(default=False, index=True)
    llm_enabled: bool = Field(default=False, index=True)
    updated_at: datetime = Field(default_factory=_now)
