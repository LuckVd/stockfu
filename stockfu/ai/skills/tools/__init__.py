...
def discover_and_register() -> None:
    ...
    for f in sorted(tools_dir.glob("*.py")):
        if f.stem == "__init__":
            continue
        mod = importlib.import_module(f"stockfu.ai.skills.tools.{f.stem}")
...
def execute_tool(name: str, code: str, **kwargs) -> str:
    """执行工具并记录调用。返回人类可读结果。"""
    tool = REGISTRY.get(name)
    if not tool:
        return f"[错误:未知工具 '{name}']"
    result = tool["execute"](code, **kwargs)
