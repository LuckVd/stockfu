"""stockfu AI 顾问共享口径宪法(operators/llm 镜像,逐字复制自 skills/constitution.py)。
...
4. 输出**严格**为单个 JSON 对象(禁止 markdown 代码块、禁止前后任何文字),字段与类型固定:
   ...
   - confidence: **0~1 的小数**(如 0.72),严禁用 low/medium/high 等词语
    - reasoning: 字符串,2-4 句(不超过500字),必须引用你依据的具体数值
   - evidence: 对象,放你引用的关键数据(键值对)
   - target_weight: (可选) 建议仓位占比 0~1,如 0.08=8%仓位;不填则由仓位管理器按信号决定
