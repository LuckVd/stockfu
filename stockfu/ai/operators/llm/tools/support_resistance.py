"""support_resistance: 支撑位/阻力位识别(operators/llm 镜像,逐字复制自 skills/tools/support_resistance.py)"""
...
def execute(code: str, lookback: int = 60, buckets: int = 10, as_of=None) -> str:
    closes = quote_series(code, "close", lookback, as_of=as_of)
    highs = quote_series(code, "high", lookback, as_of=as_of)
    lows = quote_series(code, "low", lookback, as_of=as_of)
