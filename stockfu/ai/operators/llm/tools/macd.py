"""macd: MACD 金叉/死叉/柱线/零轴位置/顶底背离(operators/llm 镜像,逐字复制自 skills/tools/macd.py)"""
...
def execute(code: str, fast: int = 12, slow: int = 26, signal: int = 9, as_of=None) -> str:
    need = slow + signal + 30
    closes = quote_series(code, "close", need, as_of=as_of)
