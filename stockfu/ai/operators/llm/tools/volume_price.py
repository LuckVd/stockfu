"""volume_price: 量价配合/背离分析(operators/llm 镜像,逐字复制自 skills/tools/volume_price.py)"""
...
def execute(code: str, lookback: int = 20, vol_threshold: float = 2.0, as_of=None) -> str:
    closes = quote_series(code, "close", lookback + 10, as_of=as_of)
    volumes = quote_series(code, "volume", lookback + 10, as_of=as_of)
