...
复用自旧 portfolio.py 的 CapAndRank 算法(只改方法名 allocate→adjust、id 名):
  - 减仓/清仓(desired <= current,含 risk_vetoed 清仓 desired=0):永远直接放行,
    释放资金、风险优先,不参与额度竞争。
  - 增仓/建仓(desired > current):按 (score×confidence, code) 降序竞争 max_gross 额度;
...
class CapAndRank(Rebalancer):
    ...
    def adjust(self, desired, current, meta, equity, params):
        ...
        # 第一遍:遍历全集(desired ∪ current)。减仓/清仓/维持 放行;增仓 进竞争池;未覆盖→维持
        for code in set(desired) | set(current):
            ...
            if d is None:
                ...
                buy_requests.append((score * conf, code, d))
