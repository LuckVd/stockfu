...
│   ├── services/        # 业务（持仓汇总/股息率/网格/历史宇宙/公司行为）
│   ├── api/             # FastAPI 路由 + 静态前端
...
- 2026-08 调研 10 策略三跑门禁：`low_beta_dividend` / `graham_defensive_value` / `smart_beta_multi_factor` 通过（smart_beta 后经持仓排查降级为参照——train 段超额是小盘风格 beta 非 alpha）；追趋势族、小盘族证伪
- 候选策略参数打磨进行中（train 段网格 → test 段样本外验证，walk-forward 纪律）
