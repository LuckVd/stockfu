# cn_momentum_cross_section 策略规格

对应配置:`stockfu/ai/strategies/cn_momentum_cross_section.yaml`。
客观规格,不含绩效。
...
| 项 | 值 |
|---|---|
| strategy_id | cn_momentum_cross_section |
...
输入 `quote_series(code, "close", 35, as_of)`(qfq),对 closes[-20:] 做价格对时间线性回归:
...
按 code 排序逐票:`max_target_step=1.0`;`min_w=max(0.005,0.01)` 时 `|current−target|<min_w` 不下单;`edge=0.005` 时 `|target−executed|<edge` 不下单;增仓 `buy_cool_down_days=1`(距上次买入<1 交易日不下单);部分减仓 `sell_cooldown_days=1`(清仓不限)。通过 → pending_target[code]=target。
...
输入:某 code,T 日 20 日 ret=+6%,20 日 r²=0.75 且 slope>0,MA 多头排列。

```
momentum:         ret=6 → score=6×2=12.0
trend_linearity:  r2=0.75, slope>0 → score=0.75×20=15.0
trend_strength:   bullish → score=20.0
total = 12.0×1.0 + 15.0×0.6 + 20.0×0.4 = 12.0 + 9.0 + 8.0 = 29.0
final_signal = strong_buy(≥ 12)
target = round(0.05 × min(29.0/8, 1), 4) = 0.05
```
