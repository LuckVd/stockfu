# momentum_breakout_cross_section 策略规格

对应配置:`stockfu/ai/strategies/momentum_breakout_cross_section.yaml`。
客观规格,不含绩效。
...
| 项 | 值 |
|---|---|
| strategy_id | momentum_breakout_cross_section |
...
基础池 = `quote_snapshot.asset_code` 去重。每个交易日 T,`eligible_on(T)` 仅用 ≤T 信息过滤:

- `as_of ≥ delist_date` → 剔除
- anchor = `list_date`(use_list_date=True),否则该 code 在 quote_snapshot 的最小 quote_date;为空 → 剔除
- `as_of < anchor` → 剔除
...
strict=True 时,U(T) 再交当日有 close 的票。板块判定:`688`→star;`300`/`301`→chinext;`8`/`4`(长度 6)→bse;其余→main。
...
月线数据 < 20 → score = 0, confidence = 0.3。命中置信度 0.4–0.8(按 |score|)。
...
```
max_target_step=1.0:  target>executed → min(target, executed+1.0)
min_w=max(0.005,0.01): |current−target|<min_w → 不下单
edge=0.005:           |target−executed|<edge → 不下单
增仓: buy_cool_down_days=1,距上次买入<1 交易日 → 不下单
部分减仓(target>0): sell_cooldown_days=1,距上次减仓<1 交易日 → 不下单;清仓不限
通过 → pending_target[code]=target
```
...
输入:某 code,T 日 monthly_bollinger position=0.20(<0.3,下轨附近),20 日 ret=+5%,MA 多头排列。

```
monthly_bollinger: position=0.20<0.3 → score=6×(1−0.20/0.3)=2.0
momentum:          ret=5 → score=5×2=10.0
trend_strength:    bullish → score=20.0
total = 2.0×1.2 + 10.0×0.6 + 20.0×0.4 = 2.4 + 6.0 + 8.0 = 16.4
final_signal = strong_buy(≥ 10)
target = round(0.05 × min(16.4/8, 1), 4) = 0.05
```
