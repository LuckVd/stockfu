# etf_momentum_rotation 策略规格

对应配置:`stockfu/ai/strategies/etf_momentum_rotation.yaml`。
客观规格,不含绩效。
...
| 参数 | 值 | 来源 |
|---|---|---|
| top_n | 5 | rebalancer_params(_TOP_N_ETF) |
| lock_days | 20 | rebalancer_params |
| max_replace | 1 | rebalancer_params |
| max_w(rebalancer) | 0.18 | rebalancer_params |
...
基础池 = `EtfQuoteDaily.asset_code` 去重。每个交易日 T:

```
U(T) = { code | code 当日有 close }
```

(strict=False:engine 用 `set(close_prices.keys())`。)板块判定:ETF 一般 main(10%),少数 20%。
...
_total_to_weight(total_score, max_w=0.18, dead=3.0, score_full=8):
    total ≤ −3     → 0.0
    −3 < total < 3 → None
    total ≥ 3      → round(0.18 × min(total/8, 1.0), 4)
```

desired 供 top_n_picker 判定建仓符号;实际持仓权重见 §8(max_w = 0.18)。
...
① 排序:ranked = codes 按 (横截面百分位(raw)×confidence, code) 降序(risk_vetoed 沉底)
   target_set = ranked[:top_n=5]
② 持仓分类(held = current>0.001):
   risk_vetoed          → final=0, 清 entry
   in target_set        → final = max_w(0.18) if desired∈{None,>0} else desired
...
```
momentum:         ret=4 → score=8.0
trend_linearity:  r2=0.65, slope>0 → score=13.0
trend_strength:   bullish → score=20.0
total = 8.0×1.0 + 13.0×0.6 + 20.0×0.4 = 23.8
desired = round(0.18 × min(23.8/8, 1), 4) = 0.18  (>0,看好)
```

进 top_n_picker:若 code ∈ ranked[:5] → final = max_w = 0.18;已持仓 lock 期内 → 维持;过 lock 且非 target → 入 replaceable。
