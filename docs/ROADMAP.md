...
**当前阶段**：MVP。数据层（多源 fallback）→ 存储 → 持仓/股息/网格 → TUI/API → 三层情绪指数 → 历史回补 → AI 4 顾问 → 四层架构回测引擎均已落地；回测基准已激活（G02）；**回测性能优化（G09）已完成**。下阶段重点在**数据缺口补全**、**板块轮动信号**与**TUI 多屏**（见 §5）。
...
| `ai/` AI 层 | 实盘 4 顾问 + 算子平台 + 选股层 | 4 常驻顾问(趋势/逆向/风险/估值)；operators(7 math + 4 llm + 2 聚合)；rebalancers(pass_through/cap_and_rank/top_n_picker)；active 走 `app_config` |
...
- **四层量化架构边界**：算子→策略→rebalancer→执行，不跳层；active 走 `app_config` 路由，不硬编码；算子签名变更必须更新 fingerprint。
...
| G03 | LLM 策略回测 | classic_4advisors / hybrid 策略可回测（纯 math 策略已可用） | 📋 | — | — | — | — | 需 LLM key |
...
- **LLM 回测**：classic_4advisors / hybrid 策略回测（G03，需 LLM key）。
...
下一阶段 P2 候选见 §5（板块轮动信号 G06 / TUI 多屏 G07 / 美股 quote 修复 G08 / 估值窗口 G04 / LLM 回测 G03），以及 `docs/ARCHITECTURE_REVIEW.md` §4 的 P2-1~P2-9（backtest 引擎演进：math 向量化 / Broker 抽象 / Sizer-CommInfo / Analyzer / Position 开平分解）。
