...
状态：设计稿。**核心架构已于 2026-08-06 落地并完成一轮正确性修复**（见 `docs/SPECS/v2-implementation-notes.md` 与 `docs/BACKTEST.md` §5）：0–100 评分契约、历史状态/checkpoint、V2 引擎（§9 时间协议）、历史沪深300宇宙接线、红利低波 vertical slice 已实现；测试与真实股票池回测的最终数字以修复后复验为准。因子全集迁移、V1 归档删除（§17）、完整 portfolio/risk 政策仍为后续阶段。
...
窗口内确认没有现金分红是有效的 0% 原始值，不属于数据缺失；只有存在现金分红但缺少可用的点时 raw 价格等计算输入时，才标记为 raw 缺失并触发相应门禁。
