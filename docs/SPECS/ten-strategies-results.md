...
- **数字为研究回测结果,用于风格对比与因子诊断,不代表策略优劣或实盘收益。** 这张表是历史的单窗口结果；正式结论必须按 `full`、`2013-2019`、`2020-2026` 三段门禁运行。**三段门禁已于 2026-08-12 执行（研究性,非 canonical）,结论见文末「三段门禁筛选结果」。**
...
## 三段门禁筛选结果（2026-08-12，研究性 / 非 canonical）

> 基于 `run-...188679`（monthly/weekly，git-dirty 时跑）+ 新跑 canonical `value_ep_bp_v2 daily`（`run-20260812-112002-663595`）。回测引擎代码在 188679 之后未改（后续提交仅 scheduler/recommend），筛选可信；**正式 canonical 准入须 git clean 重跑**（规则见 `docs/BACKTEST.md` §0.6.6/§0.6.11）。

口径同 §0.6.6 三段（full / 2013-2019 / 2020-2026），各策略取最优频率 full 段指标：

| 档 | 策略(最优频率) | full Sharpe | 年化 | 超额 | 三段方向 |
|---|---|---|---|---|---|
| 🥇 真 alpha | 多因子(monthly) | 0.99 | 16.2% | +395 | +++ |
| 🥇 | 价值 E/P+B/P(daily) | 0.81 | 15.0% | +324 | +++ |
| 🥇 | 高股息(monthly) | 0.65 | 11.9% | +174 | +++ |
| 🥈 弱 | 低波动(monthly) | 0.59 | 6.9% | +9 | +++ |
| ⚠️ 风格 | 低Beta防御(monthly) | 0.54 | 6.6% | +1 | +-+ |
| ❌ 淘汰 | 52周新高 / 动量 / 反转 / 趋势 / RSI | 0.09~0.35 | — | 负 | 失败 |

**频率甜点**：value_ep_bp 随频率单调改善(月0.64→周0.73→日0.81，daily 换手 472% 反低于 weekly 814%)；multi_factor/dividend/low_vol 高频劣化、锁 monthly（low_vol weekly 已现门禁裂缝 --+）。multi_factor daily 子段 OOM 缺失(4 因子日度全量，3.6G 无 swap)，full 已证 daily 劣。

**结论**：10 策略中仅多因子、价值、高股息具实质超额且三段稳，为后续优化目标；低Beta 是防御 beta 陷阱；后 5 个印证 A 股短期动量弱/反转接飞刀。
