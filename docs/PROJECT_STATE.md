...
### 0.6 全周期结果总表（17 策略，含 `dividend_cross_section#sl30` 变体）

> 区间 2021-01-04 → 2026-07-20（1342 交易日，base 16 行）；`#sl30` 行为 07-21 重跑（1343 交易日）。初始 100 万，前复权，股息率分母 `close_raw`。基准：base 16 行 +8.37%；`#sl30` +10.32%（多一天）。按年化降序。**勿引用 07-19/07-20 混复权 / qfq 分母旧表。** 全量 07-21 schema-2 重跑进行中（见文末），完成后统一刷新本表。
...
| 3 | 🆕 `dividend_cross_section#sl30` | 红利横截面(止损30%):base 同 config 仅 stop_loss 8%→30% | **+71.5** | **+10.65** | 16.2 | 9 | 157 | 0.10 | 1.7 | 93.6 | **+0.78** | 6 | 8.4 | 53.3 |
| 4 | `dividend_cross_section` | 红利横截面:高股息+低波+价值 | **+50.6** | **+7.99** | 14.16 | 356 | 196 | 0.20 | 3.1 | 92.0 | +0.63 | 171 | 60.6 | 47.4 |
| 5 | `cn_momentum_cross_section` | 个股动量横截面:动量+线性度+趋势 | +27.9 | +4.72 | 43.38 | 319 | 731 | 1.74 | 9.5 | 77.8 | +0.31 | 699 | 150.8 | 37.5 |
| 6 | `dual_bollinger` | 周+月双布林趋势+动量 | +25.9 | +4.43 | 42.57 | 416 | 566 | 0.89 | 17.5 | 93.4 | +0.30 | 408 | 212.4 | 42.6 |
| 7 | `etf_momentum_cross_section` | ETF动量横截面:动量+线性度+趋势 | +20.9 | +3.63 | 19.14 | 249 | 27 | 0.58 | 5.0 | 39.1 | +0.37 | 10 | 3.7 | 40.9 |
| 8 | `etf_momentum_rotation` | ETF动量轮动 | +15.0 | +2.65 | 21.55 | 691 | 27 | 0.15 | 3.6 | 24.0 | +0.31 | 7 | 7.2 | 41.2 |
| 9 | `cn_momentum_rotation` | 动量轮动(降换手)+止损 | +12.4 | +2.22 | 45.56 | 319 | 573 | 0.88 | 8.6 | 67.3 | +0.21 | 284 | 79.9 | 38.7 |
| 10 | `reversal_strategy` | 反转均值回归:短期反转+RSI超卖+低估 | +8.6 | +1.55 | 36.73 | 231 | 505 | 0.91 | 10.0 | 66.7 | +0.18 | 466 | 166.5 | 51.6 |
| 11 | `cross_section_factor` | 多因子横截面:反转+低波+价值 | +6.3 | +1.15 | 37.78 | 344 | 632 | 1.78 | 9.7 | 82.5 | +0.15 | 1237 | 222.0 | 49.6 |
| 12 | `reversal_cross_section` | 反转横截面:短期反转+RSI+低估 | +5.0 | +0.93 | 37.56 | 244 | 680 | 1.95 | 11.2 | 84.4 | +0.15 | 1217 | 251.1 | 52.9 |
| 13 | `pure_factor` | 纯因子:动量+反转+趋势+价值 | +4.8 | +0.88 | 55.16 | 未回本 | 501 | 0.91 | 11.8 | 71.7 | +0.17 | 453 | 137.3 | 42.3 |
| 14 | ❌ `bollinger_reversion_cross_section` | 布林回归横截面:日+周布林+RSI | -6.2 | -1.19 | 38.45 | 465 | 772 | 2.36 | 12.3 | 89.8 | +0.01 | 1088 | 177.3 | 50.0 |
| 15 | ❌ `dividend_low_vol` | 红利低波:高股息+低波动+低估 | -8.9 | -1.73 | 34.44 | 未回本 | 254 | 0.71 | 10.7 | 67.2 | -0.10 | 201 | 69.6 | 49.8 |
| 16 | ❌ `bollinger_reversion` | 日/周布林均值回归:触下轨买、回中上轨卖 | -12.9 | -2.55 | 39.71 | 未回本 | 610 | 0.97 | 17.5 | 79.1 | -0.08 | 324 | 93.5 | 51.4 |
| 17 | ❌ `macd_cross` | MACD金叉买/死叉卖 | -14.4 | -2.88 | 37.99 | 未回本 | 220 | 0.93 | 14.5 | 44.2 | -0.22 | 47 | 7.5 | 41.7 |
...
>
> **风险调整看卡玛**：`dividend_cross_section#sl30`（卡玛 0.66，全场最高）> base（0.56）> 动量双雄（0.33–0.43）——`#sl30` 仅把止损 8%→30%，少触发止损（171→6 次）、让利润奔跑，收益更高且回撤仅 16%、回本 9 天，是比 base 更优的核心仓位候选；动量双雄收益高但回撤 34–38%、+70% 集中在 2025 顺风年，宜作卫星。`momentum_breakout` 分年：2022 −14% / 2024 +10% / **2025 +70%** / 2026 +15%。
>
...
>
> 🆕 **指标集扩展（schema-2）**：引擎已产出 9 项此前 schema-1 产物缺失的指标——回本/回本状态、水下分布(权益低于运行峰值 >0/≥10/≥20/≥30% 的交易日占比)、去重买入数、止损成交/止损损失。其中**水下分布**此前哪儿都没有，补全后揭示：`macd_cross`/`pure_factor` 超 65% 的交易日水下 ≥30%（几乎永远深套）；`dividend` 系(含 `#sl30`)水下 ≥20% 恒为 0%（从不深套，核心仓最强证据）。base 16 行已用 `data/backtest/_backfill_meta_metrics.py` 从原始账本反算补写进 meta（水下/回本/买入精确，止损对为逼近值，全量重跑覆盖）。
>
> 🔄 **全量 schema-2 重跑进行中**（2026-07-23 启动，setsid 脱离会话）：`BACKTEST_PROGRESS=1 python3 main.py --update-backtests`，17 策略 end=07-21 原生产出。完成后 17 份 `upd-*-2026-07-21` schema-2 产物将统一刷新本表（含原生水下分布/止损损失）。进度日志 `data/backtest/full_rerun_20260723.log`（1% 粒度 + 每 1% 耗时）。
