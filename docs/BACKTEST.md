...
**运营注记（内存）**：2007 长窗回测曾因算子缓存全量预载（2007-2026 全区间 11.9M 行 ≈ 3.6G）超出宿主 3.7G 内存导致冻结重启；已改**滚动分块预载**（`CompiledStrategy.begin_run_cache` 只预载 250 日历日窗口、`prefetch_cache` 消费到尾部 20 日提前量内自动补块，`operator_cache.py` 零改动），峰值降到 ~1.5G 实测。长窗回测默认带进度日志（`BACKTEST_PROGRESS=0` 可关），须盯 RSS。
...
#### 0.6.6 新策略验证 · 强制三跑门禁（防过拟合）

**规则（2026-08-01 立，强制）**：每次做**新的策略回测验证**（新增策略、改参数、改算子逻辑后，准备认定结论之前），**必须**完成以下三轮回测，缺一不可：

1. **全样本**：`2007-01 → 最近可回测交易日`，覆盖完整周期（必须含 2008 金融危机段）。
2. **子样本 A**：从 2007–2026 内任取一段。
3. **子样本 B**：从 2007–2026 内再任取一段。

A、B 两段**较短的一段不得短于 5 年**（即两段均 ≥ 5 年），且应**覆盖不同的市场环境**（如一段含危机/熊市、一段含成长/牛市），不得两段都落在同一类行情——否则等于只验证一次，暴露不了行情特化。全样本 + 两段子样本 = **两轮回测、三轮跑**，任一缺失即视为验证未完成、结论无效。

**为什么必须有这三跑**：§0.6.4 已实证「全样本调参会过拟合衰减」——三候选夏普 test/train 仅 0.52–0.63，超额主要来自 2007–2016 前段，切开才暴露出 hold 样本外翻车。**只看全样本必过拟合**；只有额外拆出独立子样本，才能看出策略是否被某段行情特化。

**判据（认定标准）**：
- 三跑结论**方向一致**（超额同号、回撤可控）→ 策略成立，可进入候选 / 写入结论。
- **仅全样本好看、任一子样本翻车** → 判过拟合，**不得据此认定策略成立**；按 §0.6.4 判据（夏普衰减 + 超额方向）处置。
- 仍以**夏普衰减 + 超额方向**判过拟合，不用回撤（子样本含/不含危机时回撤不可比，见 §0.6.4）。

**执行注**：
- 长窗回测受宿主 3.7G 内存约束，一律 `setsid`/`nohup` + 进度日志（默认开） + 手动轮询；**background 回测会被 harness 清理，不可用**。
...
|---|---|---|---|---|---|
| low_beta_dividend | +59.6% | +98.6% | +9.7% | **26.3%** | ✅ 通过（回撤全表最低） |
| graham_defensive_value | +14.4% | +65.5% | +11.2% | 36.2% | ✅ 通过 |
| smart_beta_multi_factor | +378.5% | +215.1% | **+17.9%** | 40.9% | ✅ 通过（test 超额最高；train 异常高待排查） |
...
**背景**：§0.6.7 留下待办——smart_beta full/train 超额（+378.5%/+215.1%）异常高，排查是否重仓小盘吃风格红利。

**方法**：解析 3 个 run 的 `holdings_curve` 全部持仓（full 433,479 条 / train 164,890 / test 145,601），关联 `stock_basic` 成分标志（is_hs300 / is_zz500）按权重加权分类；年度收益用 `equity_curve` vs `benchmark` 对照。脚本 `.local/analyze_smart_beta.py` 可复跑。

**持仓构成（权重加权）**：

| 窗口 | 小盘(非成分) | 沪深300 | 中证500 | 未知 |
|---|---|---|---|---|
| train 2007–16 | **77.0%** | 10.9% | 4.7% | 7.4% |
| test 2017–26 | 31.4% | 40.4% | 24.7% | 3.5% |

- **train 段 2007–2013 小盘占比高达 87–94%**，2015 年骤降至 33%，此后 10–20%；test 段平均 ~25%。
- **持仓数从 2007 年 ~18 只膨胀到 2026 年 ~120 只**——不是等权 5 只集中组合，而是超分散组合（max_w 0.20 只限单票上限，引擎持续加仓摊薄）。
- 行业分布分散无集中（货币金融 11.4% / 零售 6.4% / 医药 6.4% / 地产 5.9% top4）。

**年度收益 vs 沪深300（full 窗口关键年份）**：

| 年 | 小盘占比 | 策略收益 | 沪深300 | 超额 |
|---|---|---:|---:|---:|
| 2009 | 90% | +63.1% | +89.9% | **−26.8pp** |
| 2013 | 86% | +7.0% | −7.7% | +14.7pp |
| 2014 | 70% | +38.4% | +52.2% | −13.8pp |
| 2015 | 33% | +61.1% | +2.5% | **+58.6pp** |
| 2020 | 40% | +1.6% | +25.5% | **−23.9pp** |
| 2021 | 19% | +10.8% | −6.2% | +17.0pp |
| 2022 | 17% | −3.0% | −21.3% | +18.3pp |

**结论（定论）**：

- **full/train 巨额超额 = 2007–2015 小盘风格 beta**（train 小盘权重 77%，2007–2013 高达 87–94%），与 §0.6.7 证伪的小盘族（small_cap/low_turnover_reversal/illiquidity）**同源**——换任何小盘策略都能打出这个数，**不是多因子选股 alpha**。
- **52周高因子客观上做了风格择时**：2015 年崩盘前小盘占比 87%→33% 自动切仓，避免了小盘族 test 全崩的下场；这是因子组合机制 + 运气，不是可宣传的 alpha。
