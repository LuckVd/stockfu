...
> **⚠ 上表已被 2026-08-15 复核取代（引擎修复后重跑），勿再引用旧数字。**
> 复核口径：当前引擎（含 `bb68888` 风控硬执行修复）、当前默认 deployment
> （`cn_equity_top15_daily_softlock30_v2`，日度引擎；旧表为月调 top15）、快照
> `bcf8e882afee`（数据到 2026-08-12，含财务表；旧表快照 `2ee50075f50c` 缺财务表，
> 当前引擎无条件预载财务三表，旧快照已无法复跑）、同一窗口 2021-01-01 → 2026-08-05、
> 观察窗 271、HS300 历史成分、`canonical=true`（git 6622145，dirty=false），工件
> `data/backtest/v1-core-review-20260815/*.checkpoint.json`。

| 风险配置（复核，formal 1083 日，基准 −1.08%） | 总收益 | 年化 | 最大回撤 | Sharpe | 超额 | 成交 | 风控触发(regime/刹车/止盈) |
|---|---:|---:|---:|---:|---:|---:|---:|
| `no_overlay_v1`（对照组） | +50.01% | +9.89% | 12.55% | 0.75 | +51.09% | 9834 | 0/0/0 |
| `v1_core_v1` | +14.95% | +3.30% | 11.51% | 0.37 | +16.03% | 4025 | 632/136/25 |

复核结论：
1. **旧表数字不可直接归因于引擎修复**——旧月调组合与现日度 softlock30 默认组合不同，
   成交结构（no_overlay 896→9834 笔）表明执行层配置已整体换代，新旧差异同时混入
   "引擎修复"与"组合默认变更"，无法干净归因。
2. **当前引擎下 v1_core_v1 的收益代价显著**：+14.95% vs 对照组 +50.01%（Sharpe
   0.37 vs 0.75），回撤仅从 12.55% 降到 11.51%（约 1 个百分点）。与进攻策略调参
   结论一致：**风控压仓是"收益换回撤"的零和交易**，此处性价比尤其低。
3. 风控触发真实执行（regime 632 次、刹车 136 次、止盈 25 次、止损 0 次），
   与旧表"压仓被软锁吞掉"的引擎缺陷已修复的行为一致。
4. 该策略（`dividend_low_vol_v2` + `v1_core_v1`）维持研究参照定位，不进正式保留集；
   研究结论如需引用，一律以复核表为准。
